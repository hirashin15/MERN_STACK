...
function simpleLens(obj, keysPath) {
  ...
  for (let i = 0; i < keysPath.length; i++) {
    if (currentPosition.hasOwnProperty(keysPath[i])) {
      currentValue = currentPosition[keysPath[i]];
      currentPosition = currentPosition[keysPath[i]];
    } else {
      return null;
    }
    if (currentPosition[keysPath[i]] === null) {
      break;
    }
  }
  return currentValue;
...
function getMaxServings(recipe, available) {
  let maxServings = 10000000;
  for (let ingredient in recipe) {
    let hasEnough = available[ingredient] / recipe[ingredient]
    if (hasEnough !== undefined) {
      if (maxServings > hasEnough) {
        maxServings = hasEnough;
      }
    } else return 0;
  }
  return maxServings;
